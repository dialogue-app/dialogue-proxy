...
app.post('/auth/signup', async (req, res) => {
  try {
    ...
    if(data.error) return res.json({ error: { message: data.error.message || 'Signup failed' } });
...
app.post('/auth/signin', async (req, res) => {
  try {
    ...
    const data = await r.json();
    if(data.error) return res.json({ error: { message: data.error.message || 'Sign in failed' } });
    res.json({ user: data.user, access_token: data.access_token, refresh_token: data.refresh_token });
  } catch(e) { res.status(500).json({ error: { message: e.message } }); }
});
...
app.post('/auth/verify', async (req, res) => {
  try {
    ...
    const data = await r.json();
    if(data.id) { res.json({ user: { id: data.id, email: data.email, user_metadata: data.user_metadata } }); }
    else { res.json({ error: { message: 'Invalid or expired token' } }); }
  } catch(e) { res.status(500).json({ error: { message: e.message } }); }
});
